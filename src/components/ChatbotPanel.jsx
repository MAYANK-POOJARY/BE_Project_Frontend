...
export default function ChatbotPanel({ hydrationResult, prefillRef }) {
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Hi! I'm your DermaSense AI skin expert powered by Claude. Ask me anything about skin hydration — or analyze your selfie and I'll personalize my advice to your results!" }
  ]);
  ...
  const sendMsg = async (override) => {
    ...
    setLoading(true);
    try {
      const sys = SYSTEM_BASE + (hydrationResult
        ? `\n\nUser's scan: ${hydrationResult.hydrationPercent}% hydration (${hydrationResult.status}), Moisture: ${hydrationResult.moistureLevel}, Skin: ${hydrationResult.skinType}. Personalize advice accordingly.`
        : "");
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "var(--radius-xl)", overflow: "hidden", display: "flex", flexDirection: "column", height: 600 }}>
      ...
      <div style={{ padding: "1rem 1.5rem", borderBottom: "1px solid var(--border)", background: "var(--surface2)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        <span style={{ fontSize: "0.7rem", color: "var(--text-muted)", padding: "3px 10px", border: "1px solid var(--border)", borderRadius: 100 }}>Powered by Claude</span>
      </div>
      ...
      {/* Messages */}
      <div style={{ flex: 1, overflowY: "auto", padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem", scrollbarWidth: "thin" }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start", flexDirection: m.role === "user" ? "row-reverse" : "row", animation: "slideUp 0.3s ease" }}>
            ...
            <div style={{ maxWidth: "72%", padding: "12px 16px", borderRadius: m.role === "assistant" ? "4px 16px 16px 16px" : "16px 4px 16px 16px", fontSize: "0.9rem", lineHeight: 1.65, border: "1px solid var(--border)", background: m.role === "assistant" ? "var(--surface2)" : "rgba(96,165,250,0.1)", borderColor: m.role === "assistant" ? "var(--border)" : "rgba(96,165,250,0.2)" }}>
              {m.content}
            </div>
          </div>
        ))}
