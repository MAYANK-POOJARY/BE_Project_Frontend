export default function ResultCard({ result, image, onAskChatbot, onSave }) {
  ...
  const heatmapCells = Array.from({ length: 20 * 20 }, (_, i) => {
    const x = i % 20;
    const y = Math.floor(i / 20);
    // Simulate face regions with higher dehydration signal
    const centerX = Math.abs(x - 10) / 10;
    const centerY = Math.abs(y - 10) / 10;
    const base = isDehydrated
      ? 0.3 + (1 - centerX * 0.5) * 0.7 * Math.random()
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr 1fr", gap: "1rem" }}>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
          <div style={{ padding: "0.75rem 1rem", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            <span style={{ fontSize: "0.7rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--blue)" }}>Analyzed Image</span>
          </div>
          <div style={{ padding: "0.75rem" }}>
            {image ? (
              <img src={image} alt="Analyzed" style={{ width: "100%", height: "auto", objectFit: "contain", borderRadius: 10 }} />
            ) : (
        ...
        <div style={{ padding: "1.5rem", display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: "2rem", alignItems: "start" }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", display: "inline-block", width: "100%" }}>
              {image && (
                <img src={image} alt="Heatmap base" style={{ width: "100%", height: "auto", objectFit: "contain", display: "block" }} />
              )}
              ...
              <div style={{
                ...
                {heatmapCells.map((v, i) => {
                  ...
                  const g = Math.round((1 - v) * 60);
                  return (
                    <div key={i} style={{ background: `rgba(${r},${g},${Math.round(v * 30)},${v * 0.85})` }} />
                  );
                })}
              </div>
              {!image && (
